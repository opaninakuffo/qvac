...
## Verify prebuilds only for the hosts a bundle actually links

PR: [#4478](https://github.com/tetherto/qvac/pull/4478)

```typescript
const { addons } = await verifyBundle({ projectRoot, addonsSource, hosts: ['win32-x64'] })

// New optional field: the hosts whose bundle graph links this addon, and so the
// only ones that need a prebuild. `[]` means the bundle links it nowhere;
// `undefined` means no link information, so every host is checked as before.
addons.find((addon) => addon.name === 'bare-posix')?.linkedHosts // []
```
